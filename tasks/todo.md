...
- [x] Generate and inspect one rights-safe football hero frame; save the selected asset in the frontend workspace.
- [x] Create a lightweight, single-play hero film and a reduced-motion-safe poster. (`frontend/public/brand`)
- [x] Rebuild the landing hero around the film with responsive, accessible controls and semantic Daze tokens. (`frontend/app/page.tsx`, `frontend/app/components`, `frontend/app/globals.css`)

### Verification

- [x] Verify image/video dimensions, duration, codecs, and asset sizes.
- [x] Run frontend lint/build and `git diff --check`.
- [x] Inspect 320px, 390px, and desktop layouts in light/dark themes; verify keyboard focus, playback, reduced motion, and no horizontal overflow.

### Review

#### Changed

- Generated a rights-safe, anonymous editorial football frame and preserved the source artwork in `assets/brand`.
...
- Browsers that reject muted autoplay wait for the first pointer/keyboard interaction; reduced-motion users remain on the poster.
