...
import { correctionMessage, finalResultMessage, pointImpactMessage, rankChangeMessage, TelegramClient } from "../../packages/telegram/src";
...
assert.match(rankChangeMessage({ previousRank: 5, nextRank: 2, contestUrl: "https://daze.test/contest/1" }), /moved up, #5 → #2/);
assert.match(rankChangeMessage({ previousRank: 2, nextRank: 5, contestUrl: "https://daze.test/contest/1" }), /moved down, #2 → #5/);
assert.match(finalResultMessage({ rank: 1, total: 42, payout: 150, contestUrl: "https://daze.test/contest/1" }), /rank #1, 42 points[\s\S]*Payout: 150 tokens/);
assert.doesNotMatch(finalResultMessage({ rank: 4, total: 10, payout: 0, contestUrl: "https://daze.test/contest/1" }), /Payout/);
